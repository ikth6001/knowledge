...
* 캐시 서버를 이용한 서버의 부하분산  
캐시서버는 프록시라는 구조를 사용, 데이터를 캐시에 저장하는 서버이다. 즉 액세스 동작을 중개할 때 웹서버에서 받은 데이터를 디스크에 저장해 두고 웹 서버를 대신하여 데이터를 클라이언트에 반송하는 기능이다.  
자세한 동작 구조를 보면 DNS에 도메인 명으로 캐시서버를 등록하고, 클라이언트로부터 캐시서버에 요청이 가면 캐시서버는 디스크에 해당 데이터가 있는지 먼저 확인을 한다. 그리고 만약 데이터가 없다면 헤더에 Via라는 필드를 추가한 후 웹 서버에 요청을 보낸 후 응답을 받으면 해당 데이터를 디스크에 작성 후 클라이언트에게 응답을 전달한다. 만약 데이터가 있다면 현재 디스크에 해당 데이터가 저장된 시기와 함께 헤더에 If-Modified-Since 필드를 추가하여 웹서버에 보내게 된다. 그러면 웹 서버는 데이터가 수정 안되었다면 안되었다는 신호만 보내고(웹 서버의 부하가 줄어드는 부분) 만약 변경 되었다면 변경된 데이터를 보내게 된다.  
캐시 서버를 클라이언트 측에 둘 수도 있다(트랜스페어런트 프록시 방식으로 클라이언트에서 웹서버로 흐르는 방면에 두는 프록시를 의미). 이렇게 하면 웹서버의 부하분산 뿐만 아니라 네트워크 트래픽의 부하도 줄일 수 있다는 점에서 큰 장점이 된다.

* 콘텐츠 배포 서비스  
클라이언트 캐쉬는 웹서버 운영자가 직접 관여할 수가 없다(클라이언트 소유니까..) 심지어 클라이언트가 캐시를 가지고 있다고 가정할 수도 없다. 때문에 프로바이더와 계약하여 캐시 서버를 클라이언트 측의 프로바에더에 두는 방법을 사용할 수 있다. 하지만 이 방법은 굉장히 비용 부담이 웹서버 운영자에게 클 수 밖에 없는데 이를 위해 캐시 서버를 대출해주는 콘텐츠 배포 서비스가 나오게 된다. 그렇다면 클라이언트가 이 콘텐츠 배포 서비스를 제공하는 캐시 서버에 요청을 보내도록 하려면 어떻게 해야할까?? DNS에 등록하는 방법을 사용하면 된다.  
하지만 콘텐츠 배포 서비스에 저장된 데이터가 웹 서버에서 변경되었다면?? 이런 부분을 해결하기 위해 통신을 하면 네트워크는 다시 부하가 올라가게 될 것이다. 때문에 웹 서버는 콘텐츠가 변경되면 콘텐츠 배포 서버에 해당 사항을 알리게 되어있다.
