...
데이터의 중복을 방지하고 inconsistency(불일치)를 방지하기 위해, 하나의 테이블을 여러 개의 테이블로 분리하는 것을 의미한다.
디스크 공간을 아낄 수 있지만, 조회 시엔 join을 하게 되므로 조회 성능이 많이 떨어지므로 UPDATE, INSERT, DELETE 작업을 주로 하는 테이블에 사용하는 것이 좋다.
   - 1NF : 각 row의 컬럼에 값이 한개만 있음을 보장(원자값)하는 것을 의미한다. 즉, 아담이 좋아하는 색상을 표현하기 위한 데이터를 표현할 때 아래와 같이 정규화 할 수 있다.
```
adam | red, blue -> adam | red
                    adam | blue
```
   - 2NF : 기본키 중에 특정 컬럼에만 종속된 컬럼이 없도록 하는것이다. 예를 들어 학생이 수강한 강의 데이터를 표현한다고 하면 학생 명과 수강 강의명이 기본키가 될 것이다. 하지만 만약 이 테이블에
학생의 나이 정보가 같이 있다고 한다면(나이 컬럼은 기본키의 학생 명 하고만 종속됨)?? 나이는 다음과 같이 불필요하게 두 번 나타나게 된다.
```
adam | 18 | math
adam | 18 | science
```
따라서 이런 경우 테이블을 나눠주게 된다.
```
adam | math
adam | sicence
```
...
테이블로 분리되게 된다.
   ...
   - 3NF : 기본키를 제외한 다른 컬럼이 그외 다른 컬럼을 결정할 수 있으면 안된다. 예를 들어 한 테이블에 시, 도, 우편번호 컬럼이 있다면 우편번호로 시와 도를 알 수 있기 때문에 이 테이블은 분리되어야 한다.

   - BCNF : 예를 들어 학생과 강좌 그리고 교수 컬럼이 있고 학생과 강좌가 기본키로 사용된다고 하자. 2NF와 3NF를 만족하지만 여전히 삽입 이상 또는 갱신이상이 생길 수 있다.
		- 삽입이상 : 알고리즘을 A 교수가 열었다고 하자. 하지만 수강생이 없으면 삽입을 할 수 없다.
		- 갱신이상 : 강의 담당 교수가 바뀌는 경우 강의를 듣는 모든 학생의 데이터를 바꿔야 한다.
		
* 비정규화  
정규화와 반대로 중복된 데이터를 감수하고 테이블을 합치는 것이다. 조회 쿼리의 성능을 높이기 위해 사용하는 방식이다.

* 인덱스를 걸 때 정규화, 비정규화 중 어느 것에 거는게 더 효과가 좋은가?  
[내생각] 정규화를 많이한 테이블은 Join 작업이 필수로 필요하고 이 Join 작업을 할 때 index가 없으면 크게 성능이 떨어진다. 때문에 index가 무조건 있어야 한다(적어도 Driven 테이블엔)


<br>클러스터링 / 비클러스터링 인덱스<br>  

* 클러스터링 인덱스 : 인덱스의 순서에 맞게 실제 테이블의 물리 데이터도 동일한 순서로 저장한다. 따라서 만약 id 컬럼이 클러스터 인덱스이면 데이터를 id 순서 상관 없이 insert 해도 실제로는 id의 순서에 맞게 데이터가 쌓이므로 조회 하면 id 순서대로 정렬되어 조회를 하게 된다(order by를 안해도). 그리고 이런 이유로 테이블당 클러스터 인덱스는 하나만 만들 수 있다.

* 비 클러스터링 인덱스 : 물리 데이터 순서를 조절 안하고 테이블이 저장되는 공간과 다른 공간에 인덱스 정보를 저장한다. 때문에 조회 시 먼저 비 클러스터 인덱스를 통해 실제 테이블 데이터가 저장된 영역 주소를 알아낸 후에 해당 주소를 간 후 해당 row의 컬럼 데이터를 다시 조회해야 한다(클러스터 인덱스보다 속도가 느림)

* 데이터가 골고루 쌓인 것과 비슷한 데이터가 쌓인 것에서 인덱스 효과가 어떤게 더 좋을까? 분포도가 좁은 경우에 인덱스 효과가 더 좋다. Single block io의 횟수가 줄어들 수 있다.
